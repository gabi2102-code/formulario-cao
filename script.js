...
document.getElementById('erroTelefone').textContent = 'Telefone inválido, deve ter pelo menos 8 caracteres';
 valido = false;
    ...
    if (cpfsExistentes.includes(Number(cpf))){
...
valido = false;
 ...
 document.getElementById('erroQuintal').textContent = "Apartamento não pode ter quintal";
valido = false;
 }
...
document.getElementById('erroMotivo').textContent = "Condição financeira insuficiente";
valido = false;
    }
...
if(motivo.length < 10){
document.getElementById('erroMotivo').textContent = 'Sua explicação deve ter no minimo 10 caracteres';
valido = false;
 }
